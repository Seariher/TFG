...
int main(int argc, char *argv[])
{
  ...
  double pos_final_cadera[12] = {0.0, 0.0, 0.0, 0.0};
  double pos_inicial_cadera[12] = {-0.36, 0.36, -0.36, 0.36};
  double percent;
  ...
  for(int i=1; i<=duration; i++){
    ...
    for(int j=0; j<4; j++){
        low_cmd_ros.motorCmd[3*j].mode = 10;
        low_cmd_ros.motorCmd[3*j].q = pos_inicial_cadera[j]*(1-percent) + pos_final_cadera[j]*percent;
        low_cmd_ros.motorCmd[3*j].Kp = 90;
        low_cmd_ros.motorCmd[3*j].dq = 0.0;
        low_cmd_ros.motorCmd[3*j].Kd = 5;
        low_cmd_ros.motorCmd[3*j].tau = 0;
    }
  ...
  double pos_inicial[12] = {0.0, 1.243, -2.81, -0.0, 1.243, -2.81, 0.0, 1.243, -2.81, -0.0, 1.243, -2.81};
  ...
  else if(opcion == "4" || opcion == "7" || opcion == "8"){
    tiempo = 2200;
  }
  ...
  foreach(rosbag::MessageInstance const m, view){
    ...
    if(force != NULL && iteration <= tiempo){
      //std::cout << force->ee_forces[0] << std::endl;
      ...
      if(iteration == 1){
        ...
        for(int i=1; i<=duration; i++){
          ...
          for(int i=1; i<=duration; i++){
            ...
            for(int j=0; j<4; j++){
                low_cmd_ros.motorCmd[3*j].mode = 10;
                low_cmd_ros.motorCmd[3*j].q = pos_final_cadera[j]*(1-percent) + pos_inicial_cadera[j]*percent;
                low_cmd_ros.motorCmd[3*j].Kp = 90;
                low_cmd_ros.motorCmd[3*j].dq = 0.0;
                low_cmd_ros.motorCmd[3*j].Kd = 5;
                low_cmd_ros.motorCmd[3*j].tau = 0;
